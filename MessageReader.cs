...
namespace S22.Pop3 {	
	...
	internal class MessageReader {
		...
		/// specified delegate.
		/// </summary>
		/// <param name="message">The mail message as a RFC822/MIME multi-line
		/// string.</param>
		private MessageReader(string message) {
			this.reader = new StringReader(message);
		}
		...
		private MailMessage ReadMailMessage(bool headerOnly) {
			NameValueCollection header = ReadMailHeader();
			if (headerOnly)
				return CreateMailmessage(header, "", null);
		...
		/// <summary>
		/// Takes a raw RFC822/MIME string and constructs a new MailMessage object
		/// from it.
		/// </summary>
		/// <param name="message">A string containing mail message data in
		/// RFC822/MIME format.</param>
		/// <param name="headerOnly">Set this to true, to process only the mail
		/// header information. Only the header fields of the returned MailMessage
		/// object will be initialized if this is set to true.</param>
		/// <returns>An initialized instance of the MailMessage class representing
		/// the mail message contained in the input message string.</returns>
		public static MailMessage Read(string message, bool headerOnly) {
			MessageReader reader = new MessageReader(message);

			return reader.ReadMailMessage(headerOnly);
		}
